...
function CaseStudiesContent() {
  ...
  return <><Navbar /><main className="overflow-hidden bg-[#F8F9FC]">
    ...
    <section id="case-library" className="px-6 py-12 md:py-20"><div className="mx-auto max-w-7xl"><div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end"><div><p className="text-sm font-bold uppercase tracking-[0.32em] text-[#FFCA00]">Case Library</p><h2 className="mt-4 font-manrope text-4xl font-bold tracking-[-0.04em] text-[#071A63] md:text-6xl">Pick a case study by industry.</h2></div><FilterGroup value={industry} onChange={setIndustry} /></div>
      <div className="mt-10 grid gap-6 lg:grid-cols-[0.95fr_1.35fr]"><div className="space-y-4 lg:max-h-[880px] lg:overflow-y-auto lg:pr-2">{filteredCases.map((item) => { const style = accents[item.accent]; const isActive = selected?.id === item.id; return <button type="button" key={item.id} onClick={() => setActiveId(item.id)} className={`group w-full rounded-[28px] border bg-white p-5 text-left shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl ${isActive ? "border-[#071A63] shadow-xl" : `border-slate-200 ${style.border}`}`}><div className="flex items-start justify-between gap-4"><div><span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] ${style.badge}`}>{item.industry}</span><h3 className="mt-4 text-2xl font-bold leading-tight text-[#071A63]">{item.company}</h3><p className="mt-2 text-sm font-medium text-slate-500">{item.country}</p></div><div className="text-right"><p className="text-2xl font-bold text-[#071A63]">{item.metric}</p><p className="mt-1 max-w-28 text-xs leading-5 text-slate-500">{item.metricLabel}</p></div></div><p className="mt-5 line-clamp-2 text-sm leading-6 text-slate-600">{item.description}</p></button>; })}</div>{selected && <CaseDetail item={selected} />}</div>
    </div></section><CTA />
  </main></>;
}
